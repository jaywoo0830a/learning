...
> **여기까지**: 부등식의 기본 — 찢고, 0 되는 점 찍고, 구간별 부호.
> 판별식이 음수이고 계수가 양수이면 항상 양수.
...
**규칙: $(x-a)^{짝수}$는 부호 불변. $(x-a)^{홀수}$는 부호 바뀜.**
...
> **여기까지**: 분수는 분모≠0 먼저. 고차는 모든 근 찍고 부호표.
> 절댓값 하나는 $|X|<a \leftrightarrow -a<X<a$. 두 개면 구간 쪼개기.
...
> **여기까지**: 지수·로그 부등식은 밑 크기(>1 or <1) 먼저 확인.
> 로그는 **진수 > 0**을 꼭 덧붙인다.

---

## Part D: 가우스 기호(바닥함수) 부등식
...
**핵심**: $[x] = n \;\Longleftrightarrow\; n \leq x < n+1$.
...
> **여기까지**: $[x]$ 부등식은 $t=[x]$로 치환 먼저. $t$는 정수.
> $\{x\}$는 $[0,1)$. $\lceil x \rceil = -[-x]$.
