...
The error bound in Example 1: $|f-L| \le \frac{M}{2}(x-a)^2$ with $M = \max|f''|$ — it vanishes for every $a,x$ exactly when $f''=0$ everywhere. The tangent model is perfect precisely for curvature-free functions: **all linearization error lives in $f''$.** That is why the bound is quadratic — it is the second derivative's fingerprint.
