...
//   묵시적 해족 = `curve.implicit(f(x,y))` — marching squares, 플롯 창 전체 자동 스캔
//   영역 셰이딩 = `region.betweenX(f, g, domain)` (fill_betweenx — 수직 스트립)
//   점 = `point().dot()` · 라벨 = `annotate.text` — 플러그인 없이 코어만으로 충분.
import { annotate, curve, kit, line, point, region, tex } from '@jaywoo0830a/logos';
...
function separableFamily() {
  const left = plot2d([-4, 4], [-5.5, 5.5], { size: [520, 440], axes: AX('x', 'y'), grid: { alpha: 0.25 } })
    .title('implicit family  y² = x² + C').add(
      ...
      legend([-3.8, 5.1], tex`C = 1,\,-4`, gray, 10),
  ...
  const right = plot2d([-3, 3], [-3.5, 3.5], { size: [520, 440], axes: AX('x', 'y'), grid: { alpha: 0.25 } })
    .title('family  y³ = x³ + C').add(
      ...
      legend([-2.8, 3.1], tex`C = 0,\,-8`, gray, 10),
      legend([-2.8, -3.15], tex`C = 8:\; y = \sqrt[3]{x^{3}+8}`, blue),
    );
...
function integratingFactor() {
  ...
  return plot2d([0, 3], [-0.25, 2.7], { size: [760, 480], axes: AX('x', 'y'), grid: { alpha: 0.28 } })
    .title("Integrating Factor — y' + 3y = e⁻ˣ  (§9.5 #5)").add(
      ...
      legend([0.15, 2.38], tex`C = 2`, blue),
      legend([0.42, 1.05], tex`C = 0.8`, green),
      legend([0.72, 0.44], tex`C = -0.4`, orange),
      legend([1.95, 0.33], tex`\text{steady }\tfrac{1}{2}e^{-x}`, red),
      annotate.text([1.35, 1.75]).label(tex`\text{transient }\; Ce^{-3x} \to 0`).font(10).color(gray),
    );
}
...
function mixing() {
  ...
  const right = plot2d([0, 120], [0, 240], { size: [520, 440], axes: AX('t (min)', 'A, V'), grid: { alpha: 0.28 } })
    .title('variable volume (§9.3 Ex 16)').add(
      ...
      point(0, 30).dot().color(purple).size(4.5),
      legend([72, 200], tex`V(t) = 100 + t`, gray),
      legend([52, 128], tex`A(t) = (100{+}t) - \frac{700000}{(100{+}t)^{2}}`, green),
      annotate.text([58, 32]).label(tex`\text{concentration } A/V \to 1\,\mathrm{g/L}`).font(10).color(green).bold(),
    );
...
function intervalValidity() {
  ...
  return plot2d([-1.5, 2.5], [-4.5, 5.5], { size: [760, 480], axes: AX('x', 'y'), grid: { alpha: 0.28 } })
    .title("Interval of Validity — y' = y²,  y(0) = 1  →  y = 1/(1 − x)").add(
      ...
      annotate.text([1.12, 4.7]).label(tex`\text{blow-up at } x = 1`).font(10.5).color(red).bold(),
      annotate.text([-1.35, 5.0]).label(tex`\text{valid on } (-\infty,\, 1)`).font(10.5).color(blue).bold(),
      annotate.text([1.25, -3.7]).label(tex`x > 1\ \text{: separate branch — not our solution}`).font(10).color(gray),
    );
}
